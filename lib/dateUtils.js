...
export function parseDataHoraLocal(str) {
  ...
  // Aqui, detectamos se o horário REAL (no Brasil) foi corrompido
  const realHoraBR = dataLocal.toLocaleString('pt-BR', {
    timeZone: 'America/Sao_Paulo',
  });
  const horaLocalBR = new Date(realHoraBR).getHours();

  console.log('▶️ DEBUG BR real:', realHoraBR);
  console.log(
    '▶️ Hora esperada:',
    horaEsperada,
    '| Interpretação BR:',
  ...
  if (horaLocalBR !== horaEsperada) {
    console.warn('[⚠️ CORRIGINDO TIMEZONE] Somando +3h');
